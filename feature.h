...
*/
struct feature {
	size_t weighted_size; /* The number of tiles the feature crosses, weighted for different score affectors (i.e. animals).*/ /**< The number of tiles the feature crosses, weighted for different score affectors (i.e. animals). */
	size_t tigers[PLAYER_COUNT]; /* The number of tigers for each player.*/ /**< Array representing the number of tigers each player has on the feature. */
	size_t crocodiles[PLAYER_COUNT]; /* The number of crocs for each player.*/ /**< Array representing the number of crocodiles each player has on the feature. */
	size_t neighbors[100]; /*
		An array of indices into the feature array on the game object.
		Shows what features are adjacent to this one.
	*/ /**< Array of indices into the features array on the game object. Shows what features are adjacent to this one.*/
	size_t neighbor_count; /**< The number of features that are adjacent to this feature. */
	struct slot open_slots[100];	/* Open slots for the feature. */ /**< Array of open slots around the feature.*/
	size_t slot_count;		/* Number of open slots. */ /**< Number of open slots around the feature. */
	enum edge type; /* Gametrail, Lake, Jungle, TODO: Den */ /**< Type of feature (e.g. lake, jungle, gametrail). */
};
